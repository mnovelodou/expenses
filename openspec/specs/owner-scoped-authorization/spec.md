...
Ensure every expense and account operation is authorized against the authenticated caller's identity, so that callers can only read, write, and delete resources they own. Ownership is determined by comparing a resource's stored `createdBy` field against the caller's JWT `sub` claim, with non-owned and missing resources both surfaced as 404 to avoid disclosing their existence.
