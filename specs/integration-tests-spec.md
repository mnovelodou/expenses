...
| Schema init | Reuse `classpath:/db/schema.sql` via `spring.sql.init.*` | Schema already uses `CREATE TABLE IF NOT EXISTS` — idempotent, no Flyway needed. **`spring.sql.init.mode=always` must be set explicitly in test properties** — Boot only auto-applies SQL init for embedded databases; without this override the schema is never applied against the Testcontainers Postgres and every test fails with a missing-table error. |
...
`BaseIT` will supply the datasource URL, username, and password via `@DynamicPropertySource` from the running container.

---

## Assumptions

1. **No cascade deletes.** `expenses.account_id` has a FK to `accounts` with no `ON DELETE CASCADE`. Attempting to delete an account that has expenses should return **412 Precondition Failed** — but this requires a dedicated handler to be implemented (see A18). Currently the FK violation propagates as a `DataIntegrityViolationException` → `GlobalExceptionHandler.handleDatabase` → `500 DATABASE_ERROR`. The integration test will first confirm the current 500 behavior, then be updated to 412 once the handler is in place.
...
5. **Request bodies follow the `{ "value": { ... } }` wrapper pattern** for accounts and expenses. A flat account payload (no `value` key) deserializes as `request.value() == null` and currently falls through to a generic `500` instead of a `400`. This is a real gap — test A6 covers it and the fix will be tracked separately.
...
Previously listed as "open questions" — all confirmed:

| Question | Decision |
|---|---|
| Should expense tests set up prerequisite accounts via MockMvc or direct JDBC injection? | **MockMvc** — keeps the full create path exercised and stays within the `@Transactional` boundary. |
| Should we test the FK violation (delete account with expenses)? | **Yes** — but the expected status is 412 Precondition Failed, not 500. Requires API-side implementation. |
...
src/test/resources/
└── application-test.properties          ← spring.sql.init.mode=always + schema location
```
...
Write endpoints wrap the domain object under `value`. Read endpoints return the bare object. MockMvc assertions must target the correct path.

| Endpoint | Response wrapper | Example JSON path |
|---|---|---|
| `POST /accounts` | `{ "value": { ... } }` | `$.value.accountId` |
| `GET /accounts/{id}` | bare | `$.accountId` |
| `GET /accounts/user/{userId}` | `{ "content": [...], "page": ..., ... }` | `$.content[0].accountId` |
| `PUT /accounts/{id}` | `{ "value": { ... } }` | `$.value.accountId` |
| `POST /expenses` | `{ "value": { ... } }` | `$.value.expenseId` |
| any error | `{ "code": "...", "message": "..." }` | `$.code` |

**Account body (bare — used by GET):**
```json
{
  "accountId": 1,
  "name": "Test Account",
...
**Account create/update response (wrapped — used by POST, PUT):**
```json
{
  "value": {
    "accountId": 1,
...
**Expense create response (wrapped — used by POST):**
```json
{
  "value": {
    "expenseId": 1,
...
|---|---|---|---|---|
| A1 | Happy path — creates account | none | valid account (DEBIT, USD, 1000.00) | 201, `$.value.accountId` present, `$.value.initialAmount == $.value.currentAmount == 1000.00` |
...
| A6 | Flat payload — missing `value` wrapper | none | `{ "name": "X", "accountType": "DEBIT", ... }` (no `value` key) | **currently 500** (gap: `request.value()` is null → NPE in `validateAccountName`). Test documents the current behavior; fix tracked separately to return 400. |

#### GET /accounts/{id}

| # | Scenario | Setup | Expected |
|---|---|---|---|
| A7 | Found | create account → use returned id | 200, bare body matches created account (`$.accountId`, not `$.value.accountId`) |
| A8 | Not found | none (id 999999) | 404, `code: NOT_FOUND` |

#### GET /accounts/user/{userId}

| # | Scenario | Setup | Expected |
|---|---|---|---|
| A9 | Returns accounts for user | create 2 accounts for `user-it` | 200, `totalElements: 2`, `content` has 2 items |
| A10 | Empty for unknown user | none | 200, `totalElements: 0`, `content: []` |
| A11 | Pagination — size=1, page=0 | create 2 accounts | `totalPages: 2`, `content` has 1 item |
| A12 | Pagination — size=1, page=1 | create 2 accounts | `content` has 1 item (second account) |
| A13 | User isolation | create account for `user-A`, query for `user-B` | `totalElements: 0` |

#### PUT /accounts/{id}

| # | Scenario | Setup | Expected |
|---|---|---|---|
| A14 | Happy path — updates account | create account | 200, `$.value.*` reflects new values, `$.value.initialAmount` unchanged |
| A15 | Not found | none (id 999999) | 404, `code: NOT_FOUND` |
| A16 | Missing name | create account | 400, `code: BAD_REQUEST` |

#### DELETE /accounts/{id}

| # | Scenario | Setup | Expected |
|---|---|---|---|
| A17 | Happy path — deletes account | create account | 204, subsequent GET returns 404 |
| A18 | Not found | none (id 999999) | 404, `code: NOT_FOUND` |
| A19 | Account has expenses — precondition failure | create account + create expense against it | **412 Precondition Failed** (requires API implementation — `GlobalExceptionHandler` must catch `DataIntegrityViolationException` specifically and return 412 instead of the generic 500) |
...
|---|---|---|---|---|
| E1 | Happy path | create account for `user-expense-it` | valid expense with same `createdBy` | 201, `$.value.expenseId` present, `$.value.expenseDate`, `$.value.accountId`, `$.value.amount`, `$.value.subCategory: RESTAURANT` |
...
These tests exercise state transitions across multiple HTTP calls within one transaction.
...
| Create account + expense → Delete account | POST account → POST expense → DELETE account → 412 | FK constraint surfaced as a meaningful error, not a 500 |
| Create expense → verify category derived | POST account → POST expense with `subCategory: RESTAURANT` → assert `$.value.subCategory` in response | `CategoryMapper` → DB → `ExpenseMapper` round-trip |
